...
module.exports = async s => {
    ...
    const downloadToFile = async (url, extension) => {
        const paths = path.join(process.cwd(), `BncrData/public/${randomUUID().split('-').join('') + extension}`);
        const hostURL = paths.replace("/bncr/BncrData/public/", "http://192.168.3.6:9090/public/");//改成你的网页地址
        return new Promise((resolve, reject) => {
            let stream = request(url).pipe(
                fs.createWriteStream(paths)
            );
            stream.on('finish', () => {
                resolve({ url: hostURL, path: paths });
            });
        });
    };
    ({ url: imageURL, path: localImagePath } = await downloadToFile(imageURL, '.png'));
    ({ url: audioURL, path: localAudioPath } = await downloadToFile(audioURL, '.mp3'));
    if (['tgBot'].includes(s.getFrom())) {
